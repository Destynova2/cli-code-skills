...
## AI marker audit (history cleanup)

When invoked with `--audit-markers` or when the user asks to clean AI traces from git history:

### Step 1 -- Detect

```bash
git log --all --grep="Co-authored-by.*Claude\|Co-Authored-By.*Claude\|Co-authored-by.*AI\|Generated by.*Claude\|Generated-by\|AI-assisted" --oneline
```

Report count and list affected commits.

### Step 2 -- Propose cleanup options

| Option | Risk | How |
|--------|------|-----|
| **A. Rebase interactive** (small scope) | Medium — rewrites recent history | `git rebase -i <first-bad-commit>~1` then edit each commit to remove the trailer |
| **B. filter-repo** (full history) | High — force push required | `git filter-repo --message-callback 'return re.sub(b"Co-Authored-By:.*Claude.*\\n", b"", message)'` |
| **C. Accept and move on** | None | AI trailers stay in old commits, new commits are clean. Auditors see past, not present |
| **D. Squash into fresh branch** | Medium | Create a new branch from the current state with a single clean commit, abandon old history |

### Step 3 -- Recommend based on context

| Situation | Recommended option |
|-----------|-------------------|
| < 5 affected commits, all on current branch | **A** (interactive rebase) |
| Many affected commits, pre-audit deadline | **B** (filter-repo) with coordination |
| Shared branch with multiple contributors | **C** (accept) — rewriting shared history is destructive |
| Fresh project, no external consumers | **D** (squash to fresh branch) |
